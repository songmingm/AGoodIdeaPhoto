...
import { deleteAlbumApi, getPrivateAlbumsApi } from '@/services/photoService';
...
const AlbumCard: React.FC = () => {
   ...
   * 获取全部我的私有图库
   */
  const doGetAllPrivateAlbum = async () => {
    const hide = message.loading('正在加载中...');
    try {
      const res = await getPrivateAlbumsApi();
      setAlbums(res.data);
      setLoading(false);
    } catch (e: any) {
  ...
  return albums.length > 0 ? (
    <Row wrap gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} justify={'center'}>
      {albums.map((value) => (
        <Col key={value.id}>
          <Badge.Ribbon text={value.albumType} color={'volcano'}>
            <Card
              key={value.id}
              style={{
                width: '300px',
